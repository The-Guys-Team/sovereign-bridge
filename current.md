...
| `dashboard/README.md` | the student wants, or already has, the visual dashboard (see §4.1) |
...
### 4.1 The dashboard: publish, don't design

There is one Bridge dashboard, `dashboard/equinox-6.html`, and every student gets that exact file. The rules are in `dashboard/README.md`:
...
**One-time dashboard migration.** The manifest's `migrations` also lists `equinox-6-migration` (`modules/equinox-6-migration.md`). It isn't a schema migration and doesn't change the student's files. It moves the student's dashboard to Equinox 6 and runs once, in the first chat after the update (§1.3). Skip it in the ordered schema steps above.
